...
export default function Header({ theme, onThemeToggle, onConnectivityOpen }: HeaderProps) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`text-xl font-bold ${theme === 'eco' ? 'text-gray-800' : ''}`}>AquaPercent</h1>
              <p className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>AI Intelligence</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>SECURE CONNECTION ACTIVE</span>
          </div>

          {/* System Health */}
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${systemHealth > 98 ? 'bg-green-500' : 'bg-yellow-500'} animate-pulse`}></div>
            <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>System Online</span>
          </div>
